...
model <- lm(log2(prot_count) ~ log2(rpf_edited_count), data = rpf_edited_prot)
plot(model)
rpf_edited_prot_residuals_test_case <- add_residuals(rpf_edited_prot, model, var = "resid") %>% mutate(label = ifelse(!between(resid,-5,5),"out","in")) # final pipe returns the outliers!
