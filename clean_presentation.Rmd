...
residual_outlier_finder <- function(res_outliers,df,vector_y,vector_x){
  model_func <- lm(log2(df[[vector_y]]) ~ log2(df[[vector_x]]), data = df)
  return(add_residuals(df,model_func,var = "resid") %>% filter(!between(resid,-res_outliers,res_outliers)))
}

str(rpf_edited_prot$prot_count)
boy <- residual_outlier_finder(res_outliers = 3, df = rpf_edited_prot, vector_y = "prot_count", vector_x = "rpf_edited_count")
